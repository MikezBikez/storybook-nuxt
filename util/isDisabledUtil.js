  ...
  methods: {
    isDisabled (property, documentType) {
      ...
      if (documentType === 'ss32') {
        if (!property.isDraftContractApprovedCompleted || !(this.isUserCustomer || this.isUserAdmin || this.isUserChiefPracticeManager)) {
          isDisabled = true
        }
      } else if (documentType === 'approve-draft-contract-draft-section-32') {
        if (property.isDraftContractApprovedCompleted || !(property.isDraftContract && property.isSection32) || !(this.isUserCustomer || this.isUserAdmin || this.isUserChiefPracticeManager)) {
          isDisabled = true
        }
      }
      return isDisabled
    ...
    isDisabledClass (property, documentType) {
      return this.isDisabled(property, documentType) ? 'disabled' : 'btn-se-primary'
    },
    isUserCustomer () {
